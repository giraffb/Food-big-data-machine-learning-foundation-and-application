...
class customSingleStageDetector(customBaseDetector):
    ...
    def _init_layers(self):
        ...
        # self.fusion_BNs = nn.ModuleList([
        #     nn.BatchNorm2d(num_features=48),
        #     nn.BatchNorm2d(num_features=136),
        #     nn.BatchNorm2d(num_features=384),
        #     # nn.Conv2d(4096, 2048, 1)
        # ])
        self.fusion_EAMs = nn.ModuleList([
            # [EMA(48,factor=16) for i in range(3)],
            # [EMA(136,factor=34 ) for i in range(3)],
            # [EMA(384,factor=32)for i in range(3)],
            # EMA(2048, factor=32),
            nn.ModuleList([EMA(2048, factor=32) for i in range(2)])
    ...
    def loss(self, batch_inputs: Tensor,
        ...
        # print("batch_inputs_d========================",batch_inputs_d.shape)
        ...
        for i in range(len(x)):
            ...
            #     concat[:, j * 2, :, :] = x[i][:, j, :, :]
            #     concat[:, j * 2 + 1, :, :] = x_d[i][:, j, :, :]
            # print("============x_f.shape", concat.shape)

            concat = self.fusion_convs[i](concat)
            # concat = self.fusion_BNs[i](concat)
            for EAM in self.fusion_EAMs[i]:
                concat = EAM(concat)
                # concat = self.fusion_EAMs[i](concat)
    ...
    def predict(self,
        ...
        for i in range(len(x)):
            # if i == 1:
            #     continue
            concat = torch.cat((x[i].detach(), x_d[i].detach()), dim=1)
