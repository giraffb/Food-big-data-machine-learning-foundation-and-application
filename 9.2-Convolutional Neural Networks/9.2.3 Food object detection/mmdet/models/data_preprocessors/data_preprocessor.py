...
class customDetDataPreprocessor(ImgDataPreprocessor):
    ...
    def forward(self, data: dict, training: bool = False) -> dict:
        ...
        inputs_d = torch.stack(data['inputs_d']).permute(0, 3, 1, 2).contiguous()
        # print(inputs.shape)
        # print(inputs_d.shape)

        # inputs_d = data['inputs_d']
        # print(data.keys())
        # print(inputs)
        ...
        if training and self.batch_augments is not None:
            for batch_aug in self.batch_augments:
                ...
                # print(len(inputs_d))
                # print(inputs_d[0].shape)
                inputs_d, data_samples = batch_aug(inputs_d, data_samples)

        return {'inputs': inputs, 'inputs_d': inputs_d, 'data_samples': data_samples}
