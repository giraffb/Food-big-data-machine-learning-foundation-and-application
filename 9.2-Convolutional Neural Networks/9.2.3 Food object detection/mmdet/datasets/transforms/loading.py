...
class customLoadImageFromFile(BaseTransform):
    ...
    def transform(self, results: dict) -> Optional[dict]:
        ...
        filename_d = results['img_path_d']
        # print(filename_d)
        # filename = str()
        filename_d = filename.split('2017', 1)[0] + '2017'+ filename_d.split('2017', 1)[1]
        ...
        try:
            if self.file_client_args is not None:
                ...
                file_client_d = fileio.FileClient.infer_client(
                    self.file_client_args, filename_d)
                img_bytes_d = file_client_d.get(filename_d)

            else:
                img_bytes = fileio.get(
                    filename, backend_args=self.backend_args)
                img_bytes_d = fileio.get(
                    filename_d, backend_args=self.backend_args)

            img = mmcv.imfrombytes(
                img_bytes, flag=self.color_type, backend=self.imdecode_backend)
            img_d = mmcv.imfrombytes(
                img_bytes_d, flag=cv2.IMREAD_GRAYSCALE, backend=self.imdecode_backend)
        ...
        img_d = np.repeat(img_d[:, :, np.newaxis], 3, axis=2)
        img_d = img_d.astype(np.float32)

        new_size = (608, 480)
        img_d_resized = cv2.resize(img_d, new_size)

        img_d_t = torch.from_numpy(img_d_resized).float()
