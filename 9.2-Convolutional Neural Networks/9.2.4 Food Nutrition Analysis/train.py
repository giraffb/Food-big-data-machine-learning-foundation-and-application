...
if __name__ == '__main__':
    ...
    # model = nn.DataParallel(model, device_ids=[0, 1], output_device=device)

    # model = load_model('resnnet50', pretrain=True, require_grad=True, pretrained_model=args.weight_path, num_class=NUM_CATEGORIES)
...
#    print("-------------------crition")
    ...
    # lr_scheduler = StepLR(optimizer, config['step_size'], config['gamma'])
    ...
    for epoch in tqdm(range(config['epochs'])):
        ...
        if val_loss and (val_loss < best_val_loss) or (not config['save_best_model_only']):
            ...
            torch.save(state_dict, epoch_dir + "/state_dict" + str(epoch) + ".pth")
            if not issave:
                issave = 1
