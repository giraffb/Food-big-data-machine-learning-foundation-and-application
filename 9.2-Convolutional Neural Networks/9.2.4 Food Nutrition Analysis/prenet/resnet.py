...
class ResNet(nn.Module):
    ...
    def forward(self, x):
        ...
        x5 = self.layer4(x4)
        # print("x5 shape is : {}".format(x5.shape))

        # error
        # print(self.layer3xx2)
        # x4_1 = self.layer3xx2(x5)
        x4_1 = self.layer3xx2(x3) #original
        # print("x4_1 shape is : {}".format(x4_1.shape))

        x5_1 = self.layer4xx2(x4_1)
        # print("x5_1 shape is : {}".format(x5_1.shape))
...
def _resnet(arch, inplanes, planes, pretrained, progress, path, **kwargs):
    ...
    if pretrained:
        ...
        model = load_pretrained(model, path=path)
...
def resnet50(pretrained=False, progress=True, path=model_urls["resnet50"], **kwargs):
