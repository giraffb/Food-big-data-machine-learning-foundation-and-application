...
class SwinTransformer(nn.Module):
    def __init__(self, downsapmle_size=4, in_channels=3, num_classes=100, embed_dim=96, depths=(2, 2, 6, 2), num_heads=(3, 6, 12, 24), window_size=7, mlp_ratio=4.,
        ...
        # there are three conv1 between stage_i and stage_i+1 in the fusion mudule
        self.conv1_stages = nn.ModuleList()
        #[192, 384, 768, 768]
        self.conv1_stages.append(nn.Conv2d(in_channels=768, out_channels=768, kernel_size=1, stride=1, padding=0))
        self.conv1_stages.append(nn.Conv2d(in_channels=384, out_channels=768, kernel_size=1, stride=1, padding=0))
        self.conv1_stages.append(nn.Conv2d(in_channels=192, out_channels=768, kernel_size=1, stride=1, padding=0))
    ...
    def forward(self, x):
        ...
        for layer in self.layers:
            ...
            # B * (H*W) * C --> B*H*W*C
            x = x.view(b, H, W, c)
            # B*H*W*C   -->  B*C*H*W for Upsample
            features.append(x.permute(0, 3, 1, 2))
            # B*C*H*W  --> B*H*W*C for next stage
            x = x.view(b, H * W, c)
        ...
        while(len(features)):
            ...
            if i:
                X = self.Upsample(X)
            fx = self.conv1_stages[i](fx)
            # fuse multi-level feature maps
            X = X + fx
            # cat tensor B*768*1*1 + B*768*1*1
            res = torch.cat((res, self.GAP[i](X)), dim=1)
            i += 1
        # B * 3072 * 1 * 1  -->  B * 3072
...
class Swin_Nutrition(nn.Module):
    def __init__(self, tasks, use_end_relus=True):
        ...
        self.base_model = SwinTransformer()
...
if __name__ == '__main__':
    ...
    task = ['1','2','3','4','5']
    model = Swin_Nutrition(tasks=task)
    out = model(x)
    print(out)

    # GAV = nn.AdaptiveAvgPool1d(1)
    # x = torch.randn(2,768,196)
    # # x =
    # x = GAV(x)
    # x = torch.flatten(x, 1)
    # fc = nn.Linear(768, 4096)
    # print(x.shape)
    # x = fc(x)
    # print(x.shape)
